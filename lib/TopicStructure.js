...
class TopicStructure {
    ...
    constructor (topic) {
        ...
        if (splittedTopic[0] === 'dh') {
            ...
            me.network = CONST.MQTT.WILDCARDS.includes(network) ? [] : [network];
            me.device = CONST.MQTT.WILDCARDS.includes(device) ? [] : [device];
            me.name = CONST.MQTT.WILDCARDS.includes(name) ? [] : [name];
        }
    }
