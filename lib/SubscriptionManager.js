...
class SubscriptionManager extends EventEmitter {
    ...
    addSubjectSubscriber (subject, subscriber, subscriptionId) {
        debug(`${this.addSubjectSubscriber.name}: ${subject} ${subscriber}, ${subscriptionId}`);

        if (this.hasSubscription(subject)) {
            this.subscriptionEntryMap.get(subject).addSubscriber(subscriber, subscriptionId);
        } else {
            this.subscriptionEntryMap.set(subject, new SubscriptionEntry(subscriber, subscriptionId));
        }
    }
        ...
        if (!this.subscriptionExecutorMap.has(subject)) {
            ...
            this.subscriptionExecutorMap.set(subject, () => {
                if (this.subscriptionEntryMap.has(subject)) {
                    ...
                    if (this.getExecutionCounter(subject) === subscriptionEntry.getSubscriptionCounter()) {
                        this.resetExecutionCounter(subject);
                        this.subscriptionExecutorMap.delete(subject);
                    }

                    debug(`${this.getSubscriptionExecutor.name}: ${subject} ${subscriptionEntry.getSubscriptionCounter()} ${this.getExecutionCounter(subject)}`);
                }
            });
        }
    ...
    findSubscriptionId (subscriber, subject) {
        ...
        if (this.subscriptionEntryMap.has(subject)) {
            resultSubscriptionId = this.subscriptionEntryMap.get(subject).getSubscriptionId(subscriber);
        }
    ...
    addSubscriptionAttempt (subscriber, subject) {
        ...
        if (this.subscriptionAttemptsMap.has(subscriber)) {
            ...
            this.subscriptionAttemptsMap.set(subscriber, (new Set()).add(subject));
        }
    }
    ...
    removeSubscriptionAttempt (subscriber, subject) {
        debug(`${this.removeSubscriptionAttempt.name}: ${subject} ${subscriber}`);
