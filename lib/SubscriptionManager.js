...
class SubscriptionManager extends EventEmitter {
    ...
    constructor () {
        ...
        me.subscriptionAttemptsMap = new Map();
        me.subscriptionEntryMap = new Map();
        me.subscriptionExecutorMap = new Map();
        me.subscriptionExecutCounterMap = new Map();

        debug(`${me.constructor.name}`);
    }
    ...
    addSubjectSubscriber (subject, subscriber, subscriptionId) {
        ...
        debug(`${me.addSubjectSubscriber.name}: ${subject} ${subscriber}, ${subscriptionId}`);
    ...
    removeSubjectSubscriber (subject, subscriber) {
        ...
        debug(`${me.removeSubjectSubscriber.name}: ${subject} ${subscriber}`);
        ...
        if (!me.subscriptionExecutorMap.has(subject)) {
            ...
            me.subscriptionExecutorMap.set(subject, function () {
                if (me.subscriptionEntryMap.has(subject)) {
                    ...
                    debug(`${me.getSubscriptionExecutor.name}: ${subject} ${subscriptionEntry.getSubscriptionCounter()} ${me.getExecutionCounter(subject)}`);
                }
            });
        }
    ...
    addSubscriptionAttempt (subscriber, subject) {
        ...
        debug(`${me.addSubscriptionAttempt.name}: ${subject} ${subscriber}`);
    ...
    removeSubscriptionAttempt (subscriber, subject) {
        ...
        debug(`${me.removeSubscriptionAttempt.name}: ${subject} ${subscriber}`);
