...
class WebSocketManager extends EventEmitter {
    ...
    constructor(wsURL) {
        ...
        this.wsFactory.on(`message`, (key, message) =>
    ...
    sendString(key, stringMessage) {
        ...
        this.wsFactory.getSocket(key).then((wSocket) => {
            if (this.wsFactory.hasSocket(key)) {
    ...
    close(key) {
        ...
        if (this._isLocked(key)) {
            this.once(`_unlock_${key}`, () =>
                process.nextTick(() => this.close(key))
            );
        } else {
