...
describe(SubscriptionManager.name, () => {
    const listOfMethods = [
        ...
        `addSubscriptionAttempt`,
        `getSubscriptionAttempts`,
        `removeSubscriptionAttempt`,
    ...
    describe(`Basic functionality`, () => {
        it(`SubscriptionAttempt`, () => {
            ...
            expect(
                subscriptionManager.hasSubscriptionAttempt(
                    subscriber1,
                    subject1
                )
            ).to.equal(true);
            ...
            expect(
                subscriptionManager.getSubscriptionAttempts(subscriber1)
            ).to.deep.equal([subject1, subject2]);
            expect(
                subscriptionManager.getSubscriptionAttempts(subscriber2)
            ).to.deep.equal([subject2]);
            expect(
                subscriptionManager.getSubscriptionAttempts(subscriber3)
            ).to.deep.equal([subject3]);

            subscriptionManager.removeSubscriptionAttempt(
                subscriber1,
                subject1
            );
            subscriptionManager.removeSubscriptionAttempt(
        ...
        it(`Subscriptions`, () => {
            ...
            expect(subscriptionManager.hasSubscription(subject1)).to.equal(
                true
            );
            expect(subscriptionManager.hasSubscription(subject2)).to.equal(
                true
            );
            expect(subscriptionManager.hasSubscription(subject3)).to.equal(
                true
            );
            expect(subscriptionManager.hasSubscription(subject4)).to.equal(
                true
            );
            ...
            expect(
                subscriptionManager.findSubject(subscriber3, subscriptionId4)
            ).to.equal(subject3);
            ...
            expect(
                subscriptionManager.findSubscriptionId(subscriber3, subject3)
            ).to.equal(subscriptionId4);
            expect(
                subscriptionManager.findSubscriptionId(subscriber3, 0)
            ).to.equal(``);
        ...
        it(`Execution counter`, () => {
            ...
            for (
                ...
                executionCounter <
                subscriptionManager.getSubscribers(subject1).length *
                    subject1Cycles;
                executionCounter++
            ) {
                subscriptionManager.getSubscriptionExecutor(
                    subject1,
                    subject1ExecutionHandler
                )();
            }
            ...
            for (
                let executionCounter = 0;
                executionCounter <
                subscriptionManager.getSubscribers(subject2).length *
                    subject2Cycles;
                executionCounter++
            ) {
                subscriptionManager.getSubscriptionExecutor(
                    subject2,
                    subject2ExecutionHandler
                )();
            }
            ...
            for (
                let executionCounter = 0;
                executionCounter <
                subscriptionManager.getSubscribers(subject3).length *
                    subject3Cycles;
                executionCounter++
            ) {
                subscriptionManager.getSubscriptionExecutor(
                    subject3,
                    subject3ExecutionHandler
                )();
            }
