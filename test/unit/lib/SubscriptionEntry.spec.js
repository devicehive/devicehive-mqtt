...
describe(SubscriptionEntry.name, () => {
    ...
    it(`should has next methods: ${listOfMethods.join(`, `)}`, () => {
        listOfMethods.forEach((methodName) => {
            expect(
                new SubscriptionEntry(subscriber1, subscriptionId1)[methodName]
            ).to.be.a(`Function`);
        });
    });
    ...
    describe(`Basic functionality`, () => {
        it(`should correct handle subscription entries`, () => {
            ...
            expect(
                subscriptionEntry.hasSubscriptionId(subscriptionId1)
            ).to.equal(true);
            expect(
                subscriptionEntry.hasSubscriptionId(subscriptionId2)
            ).to.equal(true);
            expect(
                subscriptionEntry.hasSubscriptionId(subscriptionId3)
            ).to.equal(true);
            ...
            expect(subscriptionEntry.getSubscriber(subscriptionId1)).to.equal(
                subscriber1
            );
            expect(subscriptionEntry.getSubscriber(subscriptionId2)).to.equal(
                subscriber2
            );
            expect(subscriptionEntry.getSubscriber(subscriptionId3)).to.equal(
                subscriber3
            );

            expect(subscriptionEntry.getSubscriptionId(subscriber1)).to.equal(
                subscriptionId1
            );
            expect(subscriptionEntry.getSubscriptionId(subscriber2)).to.equal(
                subscriptionId2
            );
            expect(subscriptionEntry.getSubscriptionId(subscriber3)).to.equal(
                subscriptionId3
            );
            ...
            expect(subscriptionEntry.hasSubscriber(subscriber1)).to.equal(
                false
            );
            expect(subscriptionEntry.hasSubscriber(subscriber2)).to.equal(
                false
            );
            expect(subscriptionEntry.hasSubscriber(subscriber3)).to.equal(
                false
            );
