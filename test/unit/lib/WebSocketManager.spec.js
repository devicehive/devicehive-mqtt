...
describe(WebSocketManager.name, () => {
    ...
    it(`should has next methods: ${listOfMethods.join(`, `)}`, () => {
        listOfMethods.forEach((methodName) => {
            expect(new WebSocketManager(WS_SERVER_URL)[methodName]).to.be.a(
                `Function`
            );
        });
    });
            ...
            return new Promise((resolve, reject) => {
                ...
                wsManager.on(`message`, (clientId, message) => {
                    if (wsManager.hasKey(clientId)) {
                        ...
                        reject(new Error("WebSocketManager has no client key"));
                    }
                });
            });
        });
        ...
        it(`should handle clients tokens`, (done) => {
            ...
            wsServer.on(`connection`, () => {
                ...
                const { accessToken, refreshToken } =
                    wsManager.getTokens(client1Id);

                expect(accessToken).to.equal(client1AccessToken);
                expect(refreshToken).to.equal(client1RefreshToken);
        ...
        before(() => {
            ...
            wsServer.on(`connection`, (ws) => {
                ...
                ws.on(`message`, (message) =>
                    ee.emit(`message`, message.toString())
                );
